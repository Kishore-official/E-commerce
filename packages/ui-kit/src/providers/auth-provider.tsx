...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshUser = useCallback(async () => {
    ...
    refreshPromiseRef.current = (async () => {
      ...
      try {
        ...
        // Guard: only update if no concurrent login/callback replaced our token
        ...
        // Only clear tokens if they haven't been replaced by a concurrent login.
        // If the stored token has changed, a fresh login already succeeded — don't wipe it.
        if (getRefreshToken() === capturedRefreshToken) {
          clearTokens();
        }
        setUser(null);
  ...
  useEffect(() => {
    // Skip auto-refresh on auth callback pages — the callback will store
    // tokens and navigate away. Running refreshUser here would consume the
    // refresh token (via rotation) before navigation completes, leaving a
    // stale consumed token in localStorage on the destination page.
