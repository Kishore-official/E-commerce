...
function getOrCreateSessionId(): string {
  ...
  if (!sessionId) {
    sessionId = generateIdempotencyKey(); // reuse UUID generator
...
export function createApiClient(baseURL?: string): AxiosInstance {
  ...
  // Request interceptor: attach token, session ID, + idempotency key
  client.interceptors.request.use((config: InternalAxiosRequestConfig) => {
    if (accessToken) {
      config.headers.Authorization = `Bearer ${accessToken}`;
    }

    // Attach session ID for guest cart identification
    ...
    async (error: AxiosError) => {
      ...
      if (
        ...
        try {
          ...
          clearTokens();
          // Don't hard-redirect — let the calling code handle the 401
          // (e.g., checkout shows error toast, auth-protected pages redirect)
...
/**
 * Resolve a potentially relative image URL (e.g. /api/v1/images/xxx)
...
export function resolveImageUrl(url: string): string {
  ...
  // Relative API path — prepend API base origin
  const apiBase = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000/api/v1';
  // Extract origin from apiBase (e.g. http://localhost:3000)
  try {
