...
const sourceFolder = 'C:\\Users\\bizzz\\Pictures\\ECOMMERCE_PRODUCT_IMAGES\\check\\GROCERY';
const uploadDir = path.join(process.cwd(), 'uploads', 'products');
const baseUrl = process.env.BASE_URL || 'http://localhost:3000';

async function uploadGroceryImages() {
  console.log('🚀 Starting grocery image upload process...\n');
    ...
    if (allProducts.length === 0) {
      ...
      for (let i = 0; i < Math.min(imageFilesFiltered.length, 10); i++) {
        ...
        const product: any = {
          ...
          name: `Grocery Product ${productNumber}`,
          slug: `grocery-product-${productNumber}-${Date.now()}`,
          description: `Premium grocery product ${productNumber}`,
          categoryId: null,
          brandId: null,
          status: 'approved',
          countryCode: 'SA',
...
uploadGroceryImages().catch(console.error);
