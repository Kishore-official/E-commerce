...
const sourceFolder = 'C:\\Users\\bizzz\\Pictures\\ECOMMERCE_PRODUCT_IMAGES\\check\\HOME_KITCHEN_TOOLS';
const uploadDir = path.join(process.cwd(), 'uploads', 'products');
const baseUrl = process.env.BASE_URL || 'http://localhost:3000';

async function uploadHomeKitchenToolsImages() {
  console.log('🚀 Starting home/kitchen/tools image upload process...\n');
    ...
    if (allProducts.length === 0) {
      ...
      for (let i = 0; i < Math.min(imageFilesFiltered.length, 10); i++) {
        ...
        const product: any = {
          ...
          name: `Home/Kitchen/Tools Product ${productNumber}`,
          slug: `home-kitchen-tools-product-${productNumber}-${Date.now()}`,
          description: `Premium home, kitchen, or tools product ${productNumber}`,
          categoryId: null,
          brandId: null,
          status: 'approved',
          countryCode: 'SA',
...
uploadHomeKitchenToolsImages().catch(console.error);
