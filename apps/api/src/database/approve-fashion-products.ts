...
async function approveFashionProducts() {
  console.log('🚀 Starting fashion product approval...\n');
  ...
  try {
    ...
    // Find all draft products in Fashion category
    ...
    console.log(`📦 Found ${products.length} draft products in Fashion category\n`);
    
    if (products.length === 0) {
      console.log('⚠️  No draft products found to approve.\n');
      return;
    }
    ...
    console.log('\n🎉 All fashion products are now visible in the storefront!\n');
...
approveFashionProducts().catch(console.error);
