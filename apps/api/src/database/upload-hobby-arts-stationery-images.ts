...
const sourceFolder = 'C:\\Users\\bizzz\\Pictures\\ECOMMERCE_PRODUCT_IMAGES\\check\\HOBBY_ARTS_STATIONERY';
const uploadDir = path.join(process.cwd(), 'uploads', 'products');
const baseUrl = process.env.BASE_URL || 'http://localhost:3000';

async function uploadHobbyArtsStationeryImages() {
  console.log('🚀 Starting hobby/arts/stationery image upload process...\n');
    ...
    if (allProducts.length === 0) {
      ...
      for (let i = 0; i < Math.min(imageFilesFiltered.length, 10); i++) {
        ...
        const product: any = {
          ...
          name: `Hobby/Arts/Stationery Product ${productNumber}`,
          slug: `hobby-arts-stationery-product-${productNumber}-${Date.now()}`,
          description: `Premium hobby, arts, or stationery product ${productNumber}`,
          categoryId: null,
          brandId: null,
          status: 'approved',
          countryCode: 'SA',
...
uploadHobbyArtsStationeryImages().catch(console.error);
