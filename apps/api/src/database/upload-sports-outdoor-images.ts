...
const sourceFolder = 'C:\\Users\\bizzz\\Pictures\\ECOMMERCE_PRODUCT_IMAGES\\check\\SPORTS_OUTDOOR';
const uploadDir = path.join(process.cwd(), 'uploads', 'products');
const baseUrl = process.env.BASE_URL || 'http://localhost:3000';

async function uploadSportsOutdoorImages() {
  console.log('🚀 Starting sports/outdoor image upload process...\n');
    ...
    if (allProducts.length === 0) {
      ...
      for (let i = 0; i < Math.min(imageFilesFiltered.length, 10); i++) {
        ...
        const product: any = {
          ...
          name: `Sports/Outdoor Product ${productNumber}`,
          slug: `sports-outdoor-product-${productNumber}-${Date.now()}`,
          description: `Premium sports or outdoor product ${productNumber}`,
          categoryId: null,
          brandId: null,
          status: 'approved',
          countryCode: 'SA',
...
uploadSportsOutdoorImages().catch(console.error);
