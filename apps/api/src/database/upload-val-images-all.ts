...
// All validation folders to process
const valFolders = [
...
async function uploadValImages() {
  console.log('🚀 Starting validation images upload process for all folders...\n');
  console.log(`📁 Processing ${valFolders.length} folders\n`);
  ...
  try {
    ...
    // Process each folder
    for (const sourceFolder of valFolders) {
...
uploadValImages().catch(console.error);
