...
const sourceFolder = 'C:\\Users\\bizzz\\Pictures\\ECOMMERCE_PRODUCT_IMAGES\\check\\BEAUTY_HEALTH';
const uploadDir = path.join(process.cwd(), 'uploads', 'products');
const baseUrl = process.env.BASE_URL || 'http://localhost:3000';

async function uploadBeautyImages() {
  ...
  try {
    ...
    // Get products from database (beauty/health category or all products)
    const products = await db.collection('products').find({
      status: { $in: ['approved', 'pending_review'] }
    }).limit(jpegFiles.length).toArray();
    ...
    if (products.length === 0) {
      ...
      for (let i = 0; i < Math.min(jpegFiles.length, 10); i++) {
        const imageFile = jpegFiles[i];
        const productNumber = imageFile.split('_')[0];
        
        const productId = uuidv4();
        const product: any = {
          id: productId,
          vendorId: vendor._id || vendor.id,
          name: `Beauty Product ${productNumber}`,
          slug: `beauty-product-${productNumber}-${Date.now()}`,
          description: `Premium beauty and health product ${productNumber}`,
          categoryId: null,
          brandId: null,
          status: 'approved',
          countryCode: 'SA',
    ...
    for (let i = 0; i < jpegFiles.length && i < products.length; i++) {
      const imageFile = jpegFiles[i];
...
uploadBeautyImages().catch(console.error);
