...
const sourceFolder = 'C:\\Users\\bizzz\\Pictures\\ECOMMERCE_PRODUCT_IMAGES\\check\\PET_SUPPLIES';
const uploadDir = path.join(process.cwd(), 'uploads', 'products');
const baseUrl = process.env.BASE_URL || 'http://localhost:3000';

async function uploadPetSuppliesImages() {
  console.log('🚀 Starting pet supplies image upload process...\n');
    ...
    if (allProducts.length === 0) {
      ...
      for (let i = 0; i < Math.min(imageFilesFiltered.length, 10); i++) {
        ...
        const product: any = {
          ...
          name: `Pet Supplies Product ${productNumber}`,
          slug: `pet-supplies-product-${productNumber}-${Date.now()}`,
          description: `Premium pet supplies product ${productNumber}`,
          categoryId: null,
          brandId: null,
          status: 'approved',
          countryCode: 'SA',
...
uploadPetSuppliesImages().catch(console.error);
